...
sed -Ei '/^(sudo )?(pacman|yay) /d' ~/.bash_history

# delete cd, rm, man, ls, etc.
sed -Ei '/^(sudo )?(cd|rm|ls|ll|exa|tree|cat|bat|man|which|[pfe]?grep|cp|mv|fd|find|pip|ncdu|ping|pactree)( |$)/d' ~/.bash_history
...
# delete cargo commands
sed -Ei '/^cargo (t|test|r|run|new) /d' ~/.bash_history
